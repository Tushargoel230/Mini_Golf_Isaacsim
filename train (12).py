...
def ball_in_hole(
    ...
    hole_pos is in LOCAL (env-relative) coordinates.
    ball position is converted to local frame before comparison.

    Returns boolean (N,) – True = terminate.
    """
    ball: RigidObject = env.scene[object_cfg.name]
    # Convert world → local by subtracting each env's origin
    ball_local = ball.data.root_pos_w[:, :3] - env.scene.env_origins
    hole_t     = torch.tensor(hole_pos, device=env.device).unsqueeze(0)
    dist_xy    = torch.linalg.norm(ball_local[:, :2] - hole_t[:, :2], dim=-1)
